...
class UserTest {
    ...
    void existsByEmail_afterInsert_shouldReturnTrue() {
        new User(UUID.randomUUID().toString(), "exists@test.com", "Test User", UserRole.CUSTOMER).insert();
    ...
    void existsByEmail_tombstone_shouldReturnTrue() {
        ...
        jdbcTemplate.update(
                ...
                userId, "tombstone@test.com", "Deleted User", "CUSTOMER", LocalDateTime.now(), LocalDateTime.now());
    ...
    void insert_shouldPersistAllFields() {
        String userId = UUID.randomUUID().toString();
        new User(userId, "newuser@test.com", "New User", UserRole.CUSTOMER).insert();

        Map<String, Object> row = jdbcTemplate.queryForMap(
                "SELECT * FROM users WHERE user_code = ? AND is_current = true", userId);
